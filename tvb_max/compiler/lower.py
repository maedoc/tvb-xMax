...
This is the stage that gives tvb-max its "any connectivity in" property.
...
def lower(spec: IRSpec, crosscoder) -> IRProgram:
    ...
    if spec.connectivity_is_latent:
        ...
        # fold matrix -> triu, encode via the cross-coder's trained weights
        w = jnp.asarray(spec.connectivity)
        i, j = jnp.triu_indices(w.shape[-1], k=1)
        triu = w[..., i, j]
        # CrossCoder.encode expects a batch; add leading axis
        u = crosscoder.encode(nlat, parc, sample=False)
        # if encode returns the whole cohort, take the mean as the "subject"
        u = jnp.asarray(u)
        if u.ndim > 1:
            u = u.mean(0)
